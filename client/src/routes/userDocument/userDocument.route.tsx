...
export const UserDocument = () => {
  ...
  return (
    <>
      <div
        className={`dark:bg-primary-black flex flex-col min-w-[50vw] bg-secondary-white border-r border-quaternary-gray dark:border-primary-gray ${styles.editor} ${preview ? "hidden" : ""} animate-fadeIn`}
      ...
      >
        <div className="flex gap-4 justify-between min-w-full px-4 py-3 dark:bg-secondary-black bg-primary-white" style={{ borderBottom: '2px solid #e0e0e0', borderTopLeftRadius: '10px', borderTopRightRadius: '10px' }}>
          ...
          <EyeIcon
            onClick={() => setPreview()}
            className="w-4 cursor-pointer md:hidden text-secondary-gray dark:text-tertiary-gray hover:text-primary-orange transition duration-300"
          />
        </div>
        <CodeEditor
          markdown={myMarkdown}
      ...
      <div
        className={`min-w-[50vw] dark:bg-primary-black bg-secondary-white border-r border-gray-100 ${styles.preview} animate-fadeIn`}
        style={{ borderRadius: '10px', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)' }}
      >
        <div className="flex justify-between w-full px-4 py-3 dark:bg-secondary-black bg-primary-white" style={{ borderBottom: '2px solid #e0e0e0', borderTopLeftRadius: '10px', borderTopRightRadius: '10px' }}>
          <h2 className="text-sm leading-4 text-default dark:text-tertiary-gray text-secondary-gray">
            PREVIEW
          </h2>
          {preview ? (
            <EyeSlashIcon
              onClick={() => setPreview()}
              className="w-4 cursor-pointer hover:text-primary-orange dark:text-tertiary-gray text-secondary-gray transition duration-300"
            />
          ) : (
            <EyeIcon
              onClick={() => setPreview()}
